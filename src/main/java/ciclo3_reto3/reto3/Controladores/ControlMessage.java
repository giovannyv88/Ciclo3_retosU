...
import ciclo3_reto3.reto3.Servicios.ServiciosMensaje;
...
public class ControlMessage {
    ...
    @GetMapping("/{id}")
    public Optional<TablaMessage> getMessage(@PathVariable("id") int messageId) {
        return servico.getMessage(messageId);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public TablaMessage save(@RequestBody TablaMessage message) {
        return servico.save(message);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public TablaMessage update(@RequestBody TablaMessage message) {
